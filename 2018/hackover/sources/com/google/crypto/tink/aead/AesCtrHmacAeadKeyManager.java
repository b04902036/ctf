...
class AesCtrHmacAeadKeyManager implements KeyManager<Aead> {
    ...
    public Aead getPrimitive(ByteString byteString) throws GeneralSecurityException {
        try {
            return getPrimitive(AesCtrHmacAeadKey.parseFrom(byteString));
        } catch (Throwable e) {
    ...
    public KeyData newKeyData(ByteString byteString) throws GeneralSecurityException {
        return (KeyData) KeyData.newBuilder().setTypeUrl("type.googleapis.com/google.crypto.tink.AesCtrHmacAeadKey").setValue(((AesCtrHmacAeadKey) newKey(byteString)).toByteString()).setKeyMaterialType(KeyMaterialType.SYMMETRIC).build();
    }

...
class ChaCha20Poly1305KeyManager implements KeyManager<Aead> {
    ...
    public Aead getPrimitive(ByteString byteString) throws GeneralSecurityException {
        try {
            return getPrimitive(ChaCha20Poly1305Key.parseFrom(byteString));
        } catch (Throwable e) {
    ...
    public KeyData newKeyData(ByteString byteString) throws GeneralSecurityException {
        return (KeyData) KeyData.newBuilder().setTypeUrl("type.googleapis.com/google.crypto.tink.ChaCha20Poly1305Key").setValue(newKey().toByteString()).setKeyMaterialType(KeyMaterialType.SYMMETRIC).build();
    }

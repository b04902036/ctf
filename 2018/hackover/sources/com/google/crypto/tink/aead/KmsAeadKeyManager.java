...
class KmsAeadKeyManager implements KeyManager<Aead> {
    ...
    public Aead getPrimitive(ByteString byteString) throws GeneralSecurityException {
        try {
            return getPrimitive(KmsAeadKey.parseFrom(byteString));
        } catch (Throwable e) {
    ...
    public KeyData newKeyData(ByteString byteString) throws GeneralSecurityException {
        return (KeyData) KeyData.newBuilder().setTypeUrl("type.googleapis.com/google.crypto.tink.KmsAeadKey").setValue(((KmsAeadKey) newKey(byteString)).toByteString()).setKeyMaterialType(KeyMaterialType.REMOTE).build();
    }

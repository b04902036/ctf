...
public final class NoSecretKeysetHandle {
    ...
    private static void validate(Keyset keyset) throws GeneralSecurityException {
        for (Key key : keyset.getKeyList()) {
            if (key.getKeyData().getKeyMaterialType() == KeyMaterialType.UNKNOWN_KEYMATERIAL || key.getKeyData().getKeyMaterialType() == KeyMaterialType.SYMMETRIC || key.getKeyData().getKeyMaterialType() == KeyMaterialType.ASYMMETRIC_PRIVATE) {

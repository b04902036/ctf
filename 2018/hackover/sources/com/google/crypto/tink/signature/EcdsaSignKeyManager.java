...
class EcdsaSignKeyManager implements PrivateKeyManager<PublicKeySign> {
    ...
    public PublicKeySign getPrimitive(ByteString byteString) throws GeneralSecurityException {
        try {
            return getPrimitive(EcdsaPrivateKey.parseFrom(byteString));
        } catch (Throwable e) {
    ...
    public MessageLite newKey(ByteString byteString) throws GeneralSecurityException {
        try {
            return newKey(EcdsaKeyFormat.parseFrom(byteString));
        } catch (Throwable e) {
    ...
    public KeyData newKeyData(ByteString byteString) throws GeneralSecurityException {
        return (KeyData) KeyData.newBuilder().setTypeUrl("type.googleapis.com/google.crypto.tink.EcdsaPrivateKey").setValue(((EcdsaPrivateKey) newKey(byteString)).toByteString()).setKeyMaterialType(KeyMaterialType.ASYMMETRIC_PRIVATE).build();
    }

    public KeyData getPublicKeyData(ByteString byteString) throws GeneralSecurityException {
        try {
            return (KeyData) KeyData.newBuilder().setTypeUrl("type.googleapis.com/google.crypto.tink.EcdsaPublicKey").setValue(EcdsaPrivateKey.parseFrom(byteString).getPublicKey().toByteString()).setKeyMaterialType(KeyMaterialType.ASYMMETRIC_PUBLIC).build();
        } catch (Throwable e) {
            throw new GeneralSecurityException("expected serialized EcdsaPrivateKey proto", e);
        }
    }

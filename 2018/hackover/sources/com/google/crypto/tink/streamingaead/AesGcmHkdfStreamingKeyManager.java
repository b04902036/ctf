...
class AesGcmHkdfStreamingKeyManager implements KeyManager<StreamingAead> {
    ...
    public StreamingAead getPrimitive(ByteString byteString) throws GeneralSecurityException {
        try {
            return getPrimitive(AesGcmHkdfStreamingKey.parseFrom(byteString));
    ...
    public MessageLite newKey(ByteString byteString) throws GeneralSecurityException {
        try {
            return newKey(AesGcmHkdfStreamingKeyFormat.parseFrom(byteString));
        } catch (Throwable e) {
    ...
    public KeyData newKeyData(ByteString byteString) throws GeneralSecurityException {
        return (KeyData) KeyData.newBuilder().setTypeUrl("type.googleapis.com/google.crypto.tink.AesGcmHkdfStreamingKey").setValue(((AesGcmHkdfStreamingKey) newKey(byteString)).toByteString()).setKeyMaterialType(KeyMaterialType.SYMMETRIC).build();
    }

...
public final class JsonKeysetReader implements KeysetReader {
    ...
    private EncryptedKeyset encryptedKeysetFromJson(JSONObject jSONObject) throws JSONException {
        ...
        if (this.urlSafeBase64) {
            urlSafeDecode = Base64.urlSafeDecode(jSONObject.getString("encryptedKeyset"));
        } else {
            urlSafeDecode = Base64.decode(jSONObject.getString("encryptedKeyset"));
        }
        return (EncryptedKeyset) EncryptedKeyset.newBuilder().setEncryptedKeyset(ByteString.copyFrom(urlSafeDecode)).setKeysetInfo(keysetInfoFromJson(jSONObject.getJSONObject("keysetInfo"))).build();
    }
    ...
    private KeyData keyDataFromJson(JSONObject jSONObject) throws JSONException {
        ...
        if (this.urlSafeBase64) {
            urlSafeDecode = Base64.urlSafeDecode(jSONObject.getString("value"));
        } else {
            urlSafeDecode = Base64.decode(jSONObject.getString("value"));
        }
        return (KeyData) KeyData.newBuilder().setTypeUrl(jSONObject.getString("typeUrl")).setValue(ByteString.copyFrom(urlSafeDecode)).setKeyMaterialType(getKeyMaterialType(jSONObject.getString("keyMaterialType"))).build();
    }
    ...
    private OutputPrefixType getOutputPrefixType(String str) throws JSONException {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("unknown output prefix type: ");
        stringBuilder.append(str);
        throw new JSONException(stringBuilder.toString());
    }
    ...
    private KeyMaterialType getKeyMaterialType(String str) throws JSONException {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("unknown key material type: ");
        stringBuilder.append(str);
        throw new JSONException(stringBuilder.toString());
    }

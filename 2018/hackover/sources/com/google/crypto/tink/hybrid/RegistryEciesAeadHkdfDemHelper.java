...
class RegistryEciesAeadHkdfDemHelper implements EciesAeadHkdfDemHelper {
    ...
    RegistryEciesAeadHkdfDemHelper(KeyTemplate keyTemplate) throws GeneralSecurityException {
        this.demKeyTypeUrl = keyTemplate.getTypeUrl();
        if (this.demKeyTypeUrl.equals("type.googleapis.com/google.crypto.tink.AesGcmKey")) {
            ...
            stringBuilder.append("unsupported AEAD DEM key type: ");
            stringBuilder.append(this.demKeyTypeUrl);
    ...
    public Aead getAead(byte[] bArr) throws GeneralSecurityException {
        if (bArr.length != getSymmetricKeySizeInBytes()) {
            ...
            return (Aead) Registry.getPrimitive(this.demKeyTypeUrl, (AesGcmKey) ((Builder) AesGcmKey.newBuilder().mergeFrom(this.aesGcmKey)).setKeyValue(ByteString.copyFrom(bArr, 0, this.symmetricKeySize)).build());
        } else if (this.demKeyTypeUrl.equals("type.googleapis.com/google.crypto.tink.AesCtrHmacAeadKey")) {

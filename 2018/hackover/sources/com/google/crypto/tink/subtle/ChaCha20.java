...
class ChaCha20 extends Snuffle {
    ...
    private int[] createInitialState(byte[] bArr, int i) {
        ...
        setSigma(obj);
        setKey(obj, this.key.getBytes());
        obj[12] = i;
        System.arraycopy(Snuffle.toIntArray(ByteBuffer.wrap(bArr)), 0, obj, 13, nonceSizeInBytes() / 4);
        return obj;
    }

    ByteBuffer getKeyStreamBlock(byte[] bArr, int i) {
        Object createInitialState = createInitialState(bArr, i);
    ...
    private static void setKey(int[] iArr, byte[] bArr) {
        Object toIntArray = Snuffle.toIntArray(ByteBuffer.wrap(bArr));
        System.arraycopy(toIntArray, 0, iArr, 4, toIntArray.length);
    }
    ...
    static void quarterRound(int[] iArr, int i, int i2, int i3, int i4) {
        ...
        iArr[i4] = Snuffle.rotateLeft(iArr[i4] ^ iArr[i], 16);
        iArr[i3] = iArr[i3] + iArr[i4];
        iArr[i2] = Snuffle.rotateLeft(iArr[i2] ^ iArr[i3], 12);
        iArr[i] = iArr[i] + iArr[i2];
        iArr[i4] = Snuffle.rotateLeft(iArr[i] ^ iArr[i4], 8);
        iArr[i3] = iArr[i3] + iArr[i4];
        iArr[i2] = Snuffle.rotateLeft(iArr[i2] ^ iArr[i3], 7);
    }
}

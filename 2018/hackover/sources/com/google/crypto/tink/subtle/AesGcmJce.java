...
public final class AesGcmJce implements Aead {
    ...
    public byte[] encrypt(byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        if (bArr.length <= 2147483619) {
            ...
            Object randBytes = Random.randBytes(12);
            System.arraycopy(randBytes, 0, obj, 0, 12);
            Cipher instance = instance();
            instance.init(1, this.keySpec, getParams(randBytes));
            if (!(bArr2 == null || bArr2.length == 0)) {
                instance.updateAAD(bArr2);
            }
            int doFinal = instance.doFinal(bArr, 0, bArr.length, obj, 12);
            if (doFinal == bArr.length + 16) {
                return obj;
            }
            doFinal -= bArr.length;
            throw new GeneralSecurityException(String.format("encryption failed; GCM tag must be %s bytes, but got only %s bytes", new Object[]{Integer.valueOf(16), Integer.valueOf(doFinal)}));
        }

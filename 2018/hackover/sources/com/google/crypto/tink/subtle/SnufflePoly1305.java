...
abstract class SnufflePoly1305 implements Aead {
    ...
    abstract Snuffle createSnuffleInstance(byte[] bArr, int i) throws InvalidKeyException;
    ...
    private void encrypt(ByteBuffer byteBuffer, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        if (byteBuffer.remaining() >= (bArr.length + this.snuffle.nonceSizeInBytes()) + 16) {
            ...
            bArr = new byte[this.snuffle.nonceSizeInBytes()];
            byteBuffer.get(bArr);
            byteBuffer.limit(byteBuffer.limit() - 16);
            if (bArr2 == null) {
                bArr2 = new byte[0];
            }
            byte[] computeMac = Poly1305.computeMac(getMacKey(bArr), macDataRfc7539(bArr2, byteBuffer));
            byteBuffer.limit(byteBuffer.limit() + 16);
    ...
    static byte[] macDataRfc7539(byte[] bArr, ByteBuffer byteBuffer) {
        ...
        int i = remaining % 16;
        i = (i == 0 ? remaining : (remaining + 16) - i) + length;
    ...
    private byte[] getMacKey(byte[] bArr) throws InvalidKeyException {
        ByteBuffer keyStreamBlock = this.macKeysnuffle.getKeyStreamBlock(bArr, 0);
        bArr = new byte[32];
        keyStreamBlock.get(bArr);
        return bArr;
    }
}

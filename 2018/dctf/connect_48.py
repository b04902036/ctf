...
x = [ord(i) for i in 'this_on_product']
y = [39, 37, 63, 135, 219, 79, 204, 195, 121, 62, 3, 2, 90, 140, 161]
z = [ord(new_payload[i]) ^ y[i] for i in range(len(y))]
print ([z[i] ^ x[i] for i in range(len(z))])
z = ''.join([chr(z[i] ^ x[i]) for i in range(len(z))])
print (z + chr(107 ^ 15 ^ ord('i')))
print (len(z))
exit()
	...
	for trying in pos:
		for length in range(256):
			...
			if('Looks' not in x):
				print (trying)
				exit()
#f2((([96, 97, 98, 99, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111])))
def f3(start, end):
	payload = chr(start)
